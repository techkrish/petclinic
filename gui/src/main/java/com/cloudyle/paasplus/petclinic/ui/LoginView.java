...
public class LoginView extends LoginForm implements View {
	...
	public LoginView(final Navigator loginNavigator,
	...
	private boolean checkLoginAndStoreUser(final String userName,
			final String password) {
		try {
			logger.debug("[checkLoginAndStoreUser] Retrieving Shiro subject");
			final Subject subject = SecurityUtils.getSubject();
			logger.debug("[checkLoginAndStoreUser] Trying to login user {}",
					userName);
			subject.login(new UsernamePasswordToken(userName, password));
		} catch (final AuthenticationException ex) {
			logger.info("[checkLoginAndStoreUser] Unable to authenticate user "
					+ userName, ex);
			return false;
		}

		Collection<Vet> user = ((PetClinicUI) UI.getCurrent())
				.getClinicService().findVetByLastName(userName);
		if (user.size() == 0) {
			logger.info("[checkLoginAndStoreUser] Unable to find user "
					+ userName);
			return false;
		}
	...
	@Override
	protected Component createContent(final TextField userNameField,
			final PasswordField passwordField, final Button loginButton) {
	...
	public void enter(final ViewChangeEvent event) {
		...
		this.loginWindow = new Window("Welcome to PaaS+");
	...
	protected void login(final String userName, final String password) {
		...
		if (checkLoginAndStoreUser(userName, password)) {
			...
			Notification
					.show("The username and/or password was incorrect. Please try again."); // TODO
																							// //
																							// i18n
		}
	}
